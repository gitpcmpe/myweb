...
Player.prototype = {
	...
	next: function(i) {
		...
		try {
			...
			//ejm_231207
			const baduk_sound = ['rsc/baduk_sound_1.wav', 'rsc/baduk_sound_2.wav', 'rsc/baduk_sound_3.wav']
			const random = Math.floor(Math.random() * baduk_sound.length);
			new playAudio(baduk_sound[random]);
		}
		catch(err) {
			this.error(err);
		}
